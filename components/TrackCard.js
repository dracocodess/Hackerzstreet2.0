...
export default function TrackCard({pi, track, desc}) {
    return <motion.div
        ...
        <Image src={`/assets/track${pi}.svg`} alt={`track${pi}`} width={200} height={200} className="h-48 w-48"/>
        <div>
            <div className="font-bold text-green-300">{track}</div>
            <div className="h-5"></div>
            <div className="font-medium text-slate-100 text-lg">{desc}</div>
        </div>
    </motion.div>
}